...
# UPDATED: Menambahkan PMU 2
PMU_CONFIGS = [
    {'name': 'PMU 1', 'ip': '172.17.3.56',   'port': 4712, 'id': 2},
    {'name': 'PMU 2', 'ip': '172.17.23.131', 'port': 4712, 'id': 1}, # Pastikan ID ini sesuai setting alat
...
class BufferManager:
    ...
    def can_sync(self, online_pmus: List[str]) -> bool:
        ...
        # Logika ini mewajibkan semua PMU di Config harus Online untuk sinkronisasi
...
def main():
    # Meningkatkan ukuran queue sedikit untuk mengakomodasi 3 PMU
    data_queue = Queue(maxsize=12000)
    stop_event = Event()
    processes = []
    
    # Otomatis membuat worker untuk setiap PMU di config
    ...
    try:
        while not stop_event.is_set():
            ...
            # Cek sinkronisasi (sekarang membutuhkan 3 PMU ready)
            if buffer_manager.can_sync(online_pmus):
                ...
                if (max(times) - min(times)) <= TIME_TOLERANCE:
                    ...
                    for name in online_pmus:
                        ...
                        # Simpan Angle (Default index 0 untuk semua, kecuali ada penyesuaian khusus)
                        ang_idx = 0 
                        point.field(f"angle_{tag}", float(p_data.phasors[ang_idx].angle))
                        
                        # Hitung dan Simpan Daya Aktif untuk SEMUA PMU (1, 2, dan 3)
            ...
            if (curr_t - last_report_time) >= REPORT_INTERVAL:
                ...
                # Menampilkan status list PMU yang sedang online
                print(f"\rFPS: {perf_mon.metrics.current_fps:4.1f} | Saved: {total_saved} | Status: {len(online_pmus)}/{len(PMU_CONFIGS)} Online", end="")
